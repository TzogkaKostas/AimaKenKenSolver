...
class Kenken(CSP):
	node_counter = 0
	check_counter = 0
	def __init__(self, size,cages_as_string):
		self.size = size
		#finding the rows and the columns of the grid
		rows = []
		temp = list(range(size))
		for i in range(size):
			rows.append(temp)
			temp = [x+size for x in temp]
		columns = list(zip(*rows))
		columns = [list(x) for x in columns]
	
		#domain
		domains = {}
		for i in range(size*size):
			domains[i] = list(range(1,size+1))

		#neightbors
		con_index = size*size
		self.neighbors = {}
			#splitting all cages e.g. cages_as_string = "2 * 0 1,5 + 2 3"
		cages_list = cages_as_string.split(",")
		for cage in cages_list: #e.g. cage = "2 * 0 1"
			c = cage.split(" ") #e.g. c = [2,*,0,1]
			n = len(c)-2
			result = int(c[0]) 
			symbol = c[1]
			if symbol == '#': #this means that this cell has only a number
				domains[int(c[2])] = [int(c[0])]
				for index,item in enumerate(columns):
					if int(c[2]) in item:
						pos = index
						break
				self.neighbors[int(c[2])] = set(rows[int(int(c[2])/size)]+columns[pos])-{int(c[2])}	
				continue
			comb = list(itertools.product('123456789',repeat=n))
			correct_comb = []
			for t in comb:
				t = tuple(map(int,t))
				if symbol == '-':
					if t[0]-t[1] == result or t[1]-t[0] == result:
						correct_comb.append(t)
				if symbol == '/':
					if t[0]/t[1] == result or t[1]/t[0] == result:
						correct_comb.append(t)
				if symbol == '+':
				...
				if symbol == '*':
					if prod(t) == result:
						correct_comb.append(t)
			domains[con_index] = correct_comb
			mylist = []
			...
			for i,cell in enumerate(mylist):
				for index,item in enumerate(columns):
					if int(cell) in item:
						pos = index
						break
				self.neighbors[int(cell)] = set(rows[int(int(cell)/size)]+columns[pos]+[con_index])-{int(cell)}			
			self.neighbors[con_index] = list(map(int,mylist))
			con_index = con_index + 1
		CSP.__init__(self, None, domains,self.neighbors,self.constraint_kenken)

	def constraint_kenken(self,A,a,B,b):
		self.check_counter += 1

		if A >= self.size*self.size: #if A or B >=size^2 means that we have a variable
			for i,x in enumerate(self.neighbors[A]): #from the binarization
				if int(x) == B:
					if a[i] == int(b):
						return True
					else:
						return False
		elif B >= self.size*self.size:
			for i,x in enumerate(self.neighbors[B]):
				if int(x) == A:
					if b[i] == int(a):
						return True	
					else:
...
if __name__ == "__main__":
	cage_8x8 = "2 - 0 8,7 - 1 2,1 - 3 4,2 - 5 6,120 * 7 15 23,96 * 9 10 18,35 * 11 12 19,2 / 13 21,3 / 14 22,1 - 16 24,2 / 17 25,15 + 20 28,3 - 26 27,24 * 29 30 31 38,8 + 32 33,1 - 34 35,10 * 36 44,336 * 37 45 46,7 - 39 47,105 * 40 41 49,1 - 42 50,5 - 43 51,1 - 48 56,2 - 52 60,1 - 53 61,504 * 54 55 62 63,16 + 57 58 59"
	cage_6x6 = "11 + 0 6,2 / 1 2,20 * 3 9,6 * 4 5 11 17,3 - 7 8,3 / 10 16,240 * 12 13 18 19,6 * 14 15,6 * 20 26,7 + 21 27 28,30 * 22 23,6 * 24 25,9 + 29 35,8 + 30 31 32,2 / 33 34"
	cage_5x5 = "15 * 0 5 10,1 - 1 2,2 / 3 8,2 / 4 9,10 + 6 7 11,3 - 12 17,45 * 13 18 23 24,1 - 14 19,9 + 15 16 20,2 / 21 22"
	cage_4x4 = "1 - 0 4,7 + 1 5 9,2 / 2 3,12 * 6 7 10,3 - 8 12,1 - 11 15,2 - 13 14"
	cage_3x3 = "2 / 0 3,2 - 1 2,3 # 4,2 / 5 8,5 + 6 7"

	size = 6 #choose the size of the problem
	cages = cage_6x6 #choose the grid

		#FC
	kenken = Kenken(size,cages)
	start_time = time.time()
	print("FC:")	
	print(backtracking_search(kenken,inference=forward_checking))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)
	kenken.node_counter = 0
	kenken.check_counter = 0

		#FC+MRV
	kenken = Kenken(size,cages)
	start_time = time.time()
	print("FC+MRV:")	
	print(backtracking_search(kenken, select_unassigned_variable=mrv, inference=forward_checking))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)
	...
	start_time = time.time()
	print("MAC:")	
	print(backtracking_search(kenken,inference=mac))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)
	...
	start_time = time.time()
	print("BT:")	
	print(backtracking_search(kenken))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)
	...
	start_time = time.time()
	print("BT+MRV:")	
	print(backtracking_search(kenken,select_unassigned_variable=mrv))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)
	...
	kenken.check_counter = 0
		...
		#Min-conflicts
	size = 3
	kenken = Kenken(size,cage_3x3)
	start_time = time.time()
	print("Min-conflicts:")	
	print(min_conflicts(kenken,100000))
	elapsed_time = time.time() - start_time
	print("time:",elapsed_time)
	print("nodes visited:",kenken.node_counter)
	print("check_counter:",kenken.check_counter)